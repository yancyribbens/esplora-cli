...
enum Commands {
    ...
    /// Gets a fee estimate by confirmation target in sat/vB
    GetFeeEstimates {},
    ...
    /// Gets recent block summaries at the tip or at height if provided
...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::Broadcast { transaction_hex } => {
            ...
            let r = blocking_client.broadcast(&tx).unwrap();
            println!("{:#?}", r);
        }
        Commands::GetHeight {} => {
